 ...
 *
 * 本脚本：
 *   1. 恢复两个被净化移除的采集节点（10.255.0.1 / 10.255.0.2，标记 is_hub/collector）
...
async function main() {
  ...
  try {
    ...
    const hubs = ['10.255.0.1', '10.255.0.2'];
    await session.run(
      `UNWIND $rows AS r
       MERGE (ip:IP {id: r.id})
    ...
    const hh = await session.run(
      `MATCH (a:IP)-[r:CONNECTS_TO]->(b:IP)
       WHERE NOT(a.id IN ['10.255.0.1','10.255.0.2','10.255.0.3'])
         AND NOT(b.id IN ['10.255.0.1','10.255.0.2','10.255.0.3'])
       RETURN count(r) AS c`
    );
