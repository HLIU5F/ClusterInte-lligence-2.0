...
class TestIPGeo:
    def test_private_10(self):
        ent = IPEntity(id="10.255.1.5", ip="10.255.1.5")
...
class TestServiceType:
    def test_web(self):
        ent = IPEntity(id="10.255.2.1", ip="10.255.2.1", ports=[80, 443])
        r = ServiceTypeEnricher().enrich(ent)
        assert r.properties["service_type"] == "Web"
        assert r.properties["service_confidence"] > 0

    def test_db(self):
        ent = IPEntity(id="10.255.2.2", ip="10.255.2.2", ports=[3306, 5432])
        r = ServiceTypeEnricher().enrich(ent)
        assert r.properties["service_type"] == "Database"

    def test_empty_ports(self):
        ent = IPEntity(id="10.255.2.3", ip="10.255.2.3", ports=[])
        r = ServiceTypeEnricher().enrich(ent)
...
class TestPortService:
    def test_known_port(self):
        ent = IPEntity(id="10.255.2.1", ip="10.255.2.1", ports=[80, 3306])
    ...
    def test_unknown_port(self):
        ent = IPEntity(id="10.255.2.1", ip="10.255.2.1", ports=[9999])
    ...
    def test_collector_ports(self):
        ent = IPEntity(id="10.255.0.1", ip="10.255.0.1", ports=[36000, 18060, 22003])
        r = PortServiceEnricher().enrich(ent)
...
class TestSubnetCIDR:
    def test_rfc1918_10(self):
        ent = SubnetEntity(id="10.255.1.0/24", cidr="10.255.1.0/24")
...
class TestAnomalyScore:
    def test_basic_score(self):
        ent = IPEntity(id="10.255.2.1", ip="10.255.2.1", degree=5, ports=[80, 443], is_hub=True)
...
class TestPipeline:
    def test_run_on_sample(self):
        nodes = [
            {"id": "10.255.1.1", "ip": "10.255.1.1", "degree": 3, "ports": [80, 443], "community": 0},
            {"id": "10.255.1.2", "ip": "10.255.1.2", "degree": 1, "ports": [3306], "community": 0},
        ]
        links = [{"source": "10.255.1.1", "target": "10.255.1.2", "weight": 1}]
    ...
    def test_enricher_stats(self):
        nodes = [{"id": "10.255.2.1", "ip": "10.255.2.1", "ports": [80]}]
        pl = Pipeline(Config.from_yaml(None))
        res = pl.run(nodes, [])
